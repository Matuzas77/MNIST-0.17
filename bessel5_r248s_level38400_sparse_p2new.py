...
def p2new_subspace(cusp, outdir: Path, started: float):
    ...
    d = d1.augment(d2)
    log(f"LOWERING_AUGMENT_END rows={d.nrows()} cols={d.ncols()}")
    del d1, d2
    gc.collect()
    ...
    # This is exactly the implementation used by Sage's new_submodule(2):
    # intersect the kernels of the two lowering maps and embed in cusp.
    p2new = cusp.submodule(kernel, check=False)
...
def sparse_restricted_t7(space, outdir: Path, started: float):
    ...
    for i, x in enumerate(space.basis()):
        ambient_x = ambient(x)
        image = operator.apply_sparse(ambient_x)
